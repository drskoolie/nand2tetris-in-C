...
void test_cpu_jump111(void)
{
	...
	// Output of ALU is +0, thus yes jump
	instruction_bits = set_instruction_bits(0b1, 0b0, 0b101010, 0b000, 0b111);
	cpu(instruction_bits, 0, &ram, &regs);
	TEST_ASSERT_EQUAL_INT(value, get_register_PC(&regs));

	// Output of ALU is +1, thus yes jump
	instruction_bits = set_instruction_bits(0b1, 0b0, 0b111111, 0b000, 0b111);
	cpu(instruction_bits, 0, &ram, &regs);
	TEST_ASSERT_EQUAL_INT(value, get_register_PC(&regs));

	// Output of ALU is -1, thus yes jump
	instruction_bits = set_instruction_bits(0b1, 0b0, 0b111010, 0b000, 0b111);
	cpu(instruction_bits, 0, &ram, &regs);
	TEST_ASSERT_EQUAL_INT(value, get_register_PC(&regs));
